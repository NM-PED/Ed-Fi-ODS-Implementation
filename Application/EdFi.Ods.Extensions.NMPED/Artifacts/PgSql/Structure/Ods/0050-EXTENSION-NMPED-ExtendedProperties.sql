...
COMMENT ON TABLE nmped.BEPProgramLanguageDescriptor IS 'This descriptor describes the approved languages used for the BEP program.';
COMMENT ON COLUMN nmped.BEPProgramLanguageDescriptor.BEPProgramLanguageDescriptorId IS 'A unique identifier used as Primary Key, not derived from business logic, when acting as Foreign Key, references the parent table.';
...
COMMENT ON TABLE nmped.FosterCustodialAuthorityDescriptor IS 'This descriptor indicates the custodial authority over a child in foster care.';
COMMENT ON COLUMN nmped.FosterCustodialAuthorityDescriptor.FosterCustodialAuthorityDescriptorId IS 'A unique identifier used as Primary Key, not derived from business logic, when acting as Foreign Key, references the parent table.';
...
COMMENT ON TABLE nmped.MedAlertDescriptor IS 'Alert indicator for a medical or health condition.';
COMMENT ON COLUMN nmped.MedAlertDescriptor.MedAlertDescriptorId IS 'A unique identifier used as Primary Key, not derived from business logic, when acting as Foreign Key, references the parent table.';
...
COMMENT ON TABLE nmped.RestraintEventExtension IS '';
COMMENT ON COLUMN nmped.RestraintEventExtension.RestraintEventIdentifier IS 'A unique number or alphanumeric code assigned to a restraint event by a school, school system, state, or other agency or entity.';
COMMENT ON COLUMN nmped.RestraintEventExtension.SchoolId IS 'The identifier assigned to a school.';
COMMENT ON COLUMN nmped.RestraintEventExtension.StudentUSI IS 'A unique alphanumeric code assigned to a student.';
COMMENT ON COLUMN nmped.RestraintEventExtension.RestraintTypeDescriptorId IS 'Type restraint employed on student.';

-- Extended Properties [nmped].[RestraintTypeDescriptor] --
COMMENT ON TABLE nmped.RestraintTypeDescriptor IS 'Type restraint employed on student.';
COMMENT ON COLUMN nmped.RestraintTypeDescriptor.RestraintTypeDescriptorId IS 'A unique identifier used as Primary Key, not derived from business logic, when acting as Foreign Key, references the parent table.';
...
COMMENT ON COLUMN nmped.StaffEducationOrganizationEmploymentAssociationExtension.LicenseNumber IS 'Staff license number.';
...
COMMENT ON COLUMN nmped.StudentEducationOrganizationAssociationStudentCharacteri_fc9b44.FosterCustodialAuthorityDescriptorId IS 'This field is an indicator for whether or not a child is in foster care and what authority
                    presides over them (CYFD, Tribal Entity, or Private Entity).';
COMMENT ON COLUMN nmped.StudentEducationOrganizationAssociationStudentCharacteri_fc9b44.GiftedStaffUSI IS 'A unique alphanumeric code assigned to a staff.';
...
COMMENT ON COLUMN nmped.StudentMigrantEducationProgramAssociationExtension.MedAlertDescriptorId IS 'Alert indicator for a medical/health condition.';
...
COMMENT ON COLUMN nmped.StudentProgramAssociationExtension.BEPProgramLanguageDescriptorId IS 'Approved languages used for the BEP program.';
COMMENT ON COLUMN nmped.StudentProgramAssociationExtension.DaysOffered IS 'Total number of days the program was offered.';
COMMENT ON COLUMN nmped.StudentProgramAssociationExtension.DaysAttended IS 'Total number of days the student attended the program.';
...
COMMENT ON TABLE nmped.StudentSpecialEducationProgramAssociationIEPDates IS 'Dates related to students IEP.';
COMMENT ON COLUMN nmped.StudentSpecialEducationProgramAssociationIEPDates.BeginDate IS 'The earliest date the student is involved with the program. Typically, this is the date the student becomes eligible for the program.';
COMMENT ON COLUMN nmped.StudentSpecialEducationProgramAssociationIEPDates.EducationOrganizationId IS 'The identifier assigned to an education organization.';
COMMENT ON COLUMN nmped.StudentSpecialEducationProgramAssociationIEPDates.IEPBeginDate IS 'The effective date of most recent IEP.';
COMMENT ON COLUMN nmped.StudentSpecialEducationProgramAssociationIEPDates.ProgramEducationOrganizationId IS 'The identifier assigned to an education organization.';
COMMENT ON COLUMN nmped.StudentSpecialEducationProgramAssociationIEPDates.ProgramName IS 'The formal name of the program of instruction, training, services, or benefits available through federal, state, or local agencies.';
COMMENT ON COLUMN nmped.StudentSpecialEducationProgramAssociationIEPDates.ProgramTypeDescriptorId IS 'The type of program.';
COMMENT ON COLUMN nmped.StudentSpecialEducationProgramAssociationIEPDates.StudentUSI IS 'A unique alphanumeric code assigned to a student.';
COMMENT ON COLUMN nmped.StudentSpecialEducationProgramAssociationIEPDates.AnnualIEPReviewDate IS 'The date of last IEP review.';
COMMENT ON COLUMN nmped.StudentSpecialEducationProgramAssociationIEPDates.ExpectedLateIEPDate IS 'Anticipated date for a late IEP.';
