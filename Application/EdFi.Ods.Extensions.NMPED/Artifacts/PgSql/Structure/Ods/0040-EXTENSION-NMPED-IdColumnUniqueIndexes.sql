CREATE UNIQUE INDEX IF NOT EXISTS UX_a13cc7_Id ON nmped.LocalEducationAgencyTransportation(Id);

CREATE UNIQUE INDEX IF NOT EXISTS UX_62d69c_Id ON nmped.StaffDevelopment(Id);

CREATE UNIQUE INDEX IF NOT EXISTS UX_ed2336_Id ON nmped.StaffEducationOrganizationVacancy(Id);

CREATE UNIQUE INDEX IF NOT EXISTS UX_3eb6d3_Id ON nmped.StudentCTEProgramAssociationCredential(Id);

CREATE UNIQUE INDEX IF NOT EXISTS UX_4d41c1_Id ON nmped.StudentEducationOrganizationAward(Id);

CREATE UNIQUE INDEX IF NOT EXISTS UX_1d3500_Id ON nmped.StudentSpecialEducationAssociationEvent(Id);

CREATE UNIQUE INDEX IF NOT EXISTS UX_1dbb21_Id ON nmped.StudentSpecialEducationProgramAssociationIEPDates(Id);

CREATE UNIQUE INDEX IF NOT EXISTS UX_190006_Id ON nmped.VehicleMileage(Id);

CREATE UNIQUE INDEX IF NOT EXISTS UX_e45f18_Id ON nmped.VehicleSnapshot(Id);

