DROP TRIGGER IF EXISTS [nmped].[nmped_LocalEducationAgencyTransportation_TR_UpdateChangeVersion]
GO
...
DROP TRIGGER IF EXISTS [nmped].[nmped_StaffEducationOrganizationVacancy_TR_UpdateChangeVersion]
GO
...
DROP TRIGGER IF EXISTS [nmped].[nmped_StudentSpecialEducationAssociationEvent_TR_UpdateChangeVersion]
GO
...
DROP TRIGGER IF EXISTS [nmped].[nmped_StudentSpecialEducationProgramAssociationIEPDates_TR_UpdateChangeVersion]
GO

CREATE TRIGGER [nmped].[nmped_StudentSpecialEducationProgramAssociationIEPDates_TR_UpdateChangeVersion] ON [nmped].[StudentSpecialEducationProgramAssociationIEPDates] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    UPDATE [nmped].[StudentSpecialEducationProgramAssociationIEPDates]
    SET ChangeVersion = (NEXT VALUE FOR [changes].[ChangeVersionSequence])
    FROM [nmped].[StudentSpecialEducationProgramAssociationIEPDates] u
    WHERE EXISTS (SELECT 1 FROM inserted i WHERE i.id = u.id);
END	
GO

DROP TRIGGER IF EXISTS [nmped].[nmped_VehicleMileage_TR_UpdateChangeVersion]
GO
...
DROP TRIGGER IF EXISTS [nmped].[nmped_VehicleSnapshot_TR_UpdateChangeVersion]
GO
