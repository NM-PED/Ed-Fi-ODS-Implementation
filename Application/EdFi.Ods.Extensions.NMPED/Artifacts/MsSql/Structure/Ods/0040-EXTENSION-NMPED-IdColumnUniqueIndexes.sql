...
BEGIN TRANSACTION
    IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'nmped.StaffEducationOrganizationVacancy') AND name = N'UX_StaffEducationOrganizationVacancy_Id')
    CREATE UNIQUE NONCLUSTERED INDEX UX_StaffEducationOrganizationVacancy_Id ON [nmped].[StaffEducationOrganizationVacancy]
    (Id) WITH (PAD_INDEX = ON, FILLFACTOR = 75, STATISTICS_NORECOMPUTE = OFF) ON [PRIMARY]
    GO
COMMIT

BEGIN TRANSACTION
    IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'nmped.StudentCTEProgramAssociationCredential') AND name = N'UX_StudentCTEProgramAssociationCredential_Id')
    CREATE UNIQUE NONCLUSTERED INDEX UX_StudentCTEProgramAssociationCredential_Id ON [nmped].[StudentCTEProgramAssociationCredential]
    (Id) WITH (PAD_INDEX = ON, FILLFACTOR = 75, STATISTICS_NORECOMPUTE = OFF) ON [PRIMARY]
    GO
COMMIT
...
BEGIN TRANSACTION
    IF NOT EXISTS (SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'nmped.StudentSpecialEducationProgramAssociationIEPDates') AND name = N'UX_StudentSpecialEducationProgramAssociationIEPDates_Id')
    CREATE UNIQUE NONCLUSTERED INDEX UX_StudentSpecialEducationProgramAssociationIEPDates_Id ON [nmped].[StudentSpecialEducationProgramAssociationIEPDates]
    (Id) WITH (PAD_INDEX = ON, FILLFACTOR = 75, STATISTICS_NORECOMPUTE = OFF) ON [PRIMARY]
    GO
COMMIT
