...
SELECT LEA.[LocalEducationAgencyId]
	   ...
	   ,CASE WHEN 
		(SELECT TOP 1 SchoolId FROM nmped_rpt.vw_Schools 
			...
			AND CharterStatusCode = 'District Charter') IS NULL THEN 'N' ELSE 'Y' END AS [LocalCharterIndicator] --[DistrictHasLocalCharters] better name but would require two SPs to be updated and many SSRS reports.
